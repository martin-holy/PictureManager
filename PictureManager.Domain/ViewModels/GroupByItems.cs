...
namespace PictureManager.Domain.ViewModels {
  public static class GroupByItems {
    private static readonly CategoryGroupM _unknownPeopleGroup = new(-1, "Unknown", Category.People, Res.IconPeopleMultiple);

    public static List<CollectionViewGroupByItem<SegmentM>> GetFoldersFromSegments(SegmentM[] segments) =>
    ...
    public static List<CollectionViewGroupByItem<SegmentM>> GetPeopleFromSegments(IEnumerable<SegmentM> segments) =>
      CollectionViewGroupByItem<SegmentM>.BuildTree<SegmentM, PersonM, string>(
        segments.Where(x => x.Person != null).Select(x => x.Person),
        x => new(Res.IconPeople, x.Name, x, GroupSegmentByPerson),
        x => x.Name);

    public static List<CollectionViewGroupByItem<PersonM>> GetKeywordsFromPeople(PersonM[] people) =>
