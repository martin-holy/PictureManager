...
public sealed class RotationDialogM : Dialog {
  ...
  public static event DataEventHandler<RealMediaItemM[]> OrientationChangedEvent = delegate { };
  ...
  private static void Open() {
    ...
    SetOrientation(Core.MediaItemsM.GetActive().OfType<RealMediaItemM>().ToArray(), (MediaOrientation)rotation);
  }

  private static void SetOrientation(RealMediaItemM[] mediaItems, MediaOrientation orientation) {
    foreach (var mi in mediaItems) {
      ...
      Core.Db.MediaItems.Modify(mi);
      mi.SetThumbSize(true);
      File.Delete(mi.FilePathCache);
    }

    OrientationChangedEvent(mediaItems);
  }
}